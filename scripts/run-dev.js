...
function observeProcess(child, label) {
  return new Promise(resolve => {
    ...
    child.once("error", error => {
      finish({
        code: 1,
        error: formatStartError(child.spawnCommand || label, child.spawnLabel || label, error)
      });
    });
...
async function main() {
  ...
  const firstExit = await Promise.race([observeProcess(server, "server"), observeProcess(client, "client")]);
  if (shuttingDown) {
    return;
  }
  ...
  if (firstExit.signal) {
    throw new Error(`${firstExit.label} exited from signal ${firstExit.signal}`);
  }
