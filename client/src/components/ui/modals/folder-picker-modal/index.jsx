...
export default function FolderPickerModal({
  ...
  const renderNode = (node, level = 0) => {
    ...
    return <React.Fragment key={node.slug}>
        <button type="button" className={`folder-picker-row${isSelected ? " selected" : ""}${disabled ? " disabled" : ""}`} style={{
  ...
  const footer = <div className="folder-picker-footer">
  ...
  return <ModalShell title={title} subtitle={subtitle} onClose={onClose} footer={footer} className="modal-folder-picker">
      <div className="folder-picker">
        <button type="button" className={`folder-picker-row${selected === "" ? " selected" : ""}${isRootBlocked ? " disabled" : ""}`} onClick={() => {
        ...
        <div className="folder-picker-list">
          {!hasFolders ? <div className="folder-picker-empty">No folders yet.</div> : tree.map(node => renderNode(node, 0))}
        </div>
      </div>
    </ModalShell>;
}
