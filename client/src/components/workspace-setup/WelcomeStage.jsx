...
const WelcomeStage = ({
  ...
  const accountSummary = (seededAccounts || []).map(account => `${account.username}/${account.password}`).join(", ") || "owner/owner, admin/admin, user/user";
      ...
      <div className="start-split-grid">
        <div className="start-action-card start-action-card-wide">
          ...
          <div className="start-action-note start-lede">
            <p>
              Use {brandTitle} to capture playbooks, decisions, quick
              references, and the policies that keep the team in sync.
            </p>
            <p>
          ...
          <div className="start-action-note">
            <p>
              When setup finishes we seed the starter accounts and a Start page
              at <code>{DEFAULT_START_PAGE_SLUG}</code>.
            </p>
            <p>
              Use the owner account to sign in for the first time, then refine
              the workspace title, icon, timezone, and navigation from
              Settings.
            </p>
          </div>
          <div className="start-actions-row">
            <button className="btn" type="button" onClick={() => onStageChange("setup")}>
              Continue to setup
            </button>
          </div>
        </div>
        <div className="start-side-stack">
          ...
          <div className="start-action-card start-action-card-compact">
            ...
            <div className="start-action-note">
              <ul className="start-action-list">
                <li>Seeded accounts are ready immediately: {accountSummary}.</li>
                <li>
